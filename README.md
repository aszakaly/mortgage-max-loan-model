...
An additive analysis layered on the closed pipeline **without modifying scripts
`01`–`05` or the model**.

**Segment error breakdown.** `segment_error.py` breaks the held-out error down
...
<580` tier is **empty** — the model never saw a sub-580 applicant.

This add-on reads the pipeline's git-ignored artifacts (`mortgage_clean.csv`,
